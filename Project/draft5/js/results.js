...
async function getMovieTitle(movieId) {
    try {
        ...
        return data.title || `Movie ${movieId}`; // RETURN title (or fallback)
    } catch (error) {
...
async function loadResults() {
    try {
        ...
        if (votesSnapshot.size !== currentVoteCount) {
            currentVoteCount = votesSnapshot.size; // update current vote count
        }
...
async function displayMovieResults(votes, totalVotes) {
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        const movieTitle = await getMovieTitle(movieId); // FETCH movie title from API
        const percentage = ((count / totalVotes) * 100).toFixed(1); // find percentage
        const barWidth = (count / maxVotes) * 100; // find bar witdh
        const isWinner = i === 0; // determine if top movie
